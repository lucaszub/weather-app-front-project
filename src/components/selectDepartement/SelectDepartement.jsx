...
const SelectDepartement = ({setDepartement}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // const response = await fetch(`http://127.0.0.1:8000/departments/departements/`); //for local api
        const response = await fetch(`http://13.36.174.69:8000/departments/departements/`); // for aws api
